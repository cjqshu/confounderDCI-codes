...
def PEHE(y, y_hat):
    ...
    """
    data = np.abs((y[:,1] - y[:,0]) - (y_hat[:,1] - y_hat[:,0]))
    PEHE_val = np.mean(data)
...
def sqrt_PEHE(y: np.ndarray, hat_y: np.ndarray) -> float:
    ...
    """
    data = ((y[:, 1] - y[:, 0]) - (hat_y[:, 1] - hat_y[:, 0])) ** 2
    s_pehe = np.sqrt(np.mean(data))
    interval = mean_confidence_interval(data)
    return s_pehe, interval
...
def sqrt_PEHE_with_diff(y: np.ndarray, hat_y: np.ndarray) -> float:
    ...
    """
    data = ((y[:, 1] - y[:, 0]) - hat_y) ** 2
    diff = np.mean(((y[:, 1] - y[:, 0]) - hat_y) ** 2)
    out = np.sqrt(diff)
    interval = mean_confidence_interval(data)
    return out, interval
...
#     Args:
#         t: treatment vector.
#         y: expected outcome. 观测到的潜在结果（一列）
#         hat_y: estimated outcome.  处理组和对照组的潜在结果估计值（两列）
#     Output:
...
def ATE_error(y: np.ndarray, hat_y: np.ndarray) -> np.ndarray:
    """
    ...
    Args:
        ...
        hat_y: estimated outcome.
    """
    data = (y[:, 1] - y[:, 0]) - (hat_y[:, 1] - hat_y[:, 0])
    out = np.abs(np.mean(y[:, 1] - y[:, 0]) - np.mean(hat_y[:, 1] - hat_y[:, 0]))
    interval = mean_confidence_interval(data)
    return out, interval
