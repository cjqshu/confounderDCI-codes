...
def cate_doubly_robust(X, y_factual, t, p):  

    from sklearn.ensemble import RandomForestRegressor

    reg_y1_dr = RandomForestRegressor(n_estimators=100, random_state=0)
    reg_y0_dr = RandomForestRegressor(n_estimators=100, random_state=0)
    reg_y1_dr.fit(X[t == 1], y_factual[t == 1])
    reg_y0_dr.fit(X[t == 0], y_factual[t == 0])

    mu1_hat_dr = reg_y1_dr.predict(X)
    mu0_hat_dr = reg_y0_dr.predict(X)

    p_hat_dr = p

    dr_y1 = mu1_hat_dr + (t / p_hat_dr) * (y_factual - mu1_hat_dr)
    dr_y0 = mu0_hat_dr + ((1 - t) / (1 - p_hat_dr)) * (y_factual - mu0_hat_dr)
    ite_hat_dr = dr_y1 - dr_y0

    return dr_y1, dr_y0, ite_hat_dr
...
if __name__ == "__main__":
    ...
    for conf_key, confounder_ids in confounder_dict_CDs.items():
        ...
        for file_idx in file_idx_list:  
            ...
            y1_hat, y0_hat, ite_hat = cate_doubly_robust(X, y_factual, t, e_fuse_lin)

            Y_true = np.stack([df["mu0"].to_numpy(float), df["mu1"].to_numpy(float)], axis=1)
            Y_hat_pm = np.stack([y0_hat, y1_hat], axis=1)
    ...
    result_dataset_ID.to_csv("results/IHDP/DR_confounderDCI_multi_runs_IHDP_dataset_ID.csv", index=False)
